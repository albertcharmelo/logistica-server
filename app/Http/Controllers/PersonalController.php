...
class PersonalController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = Persona::with(['unidad', 'cliente', 'sucursal', 'dueno', 'transporteTemporal', 'estado', 'agente'])->orderBy('id', 'desc');
        if ($search !== '') {
    ...
    public function store(PersonaStoreRequest $request)
    {
        ...
        // Alias agente -> agente_id
        if (array_key_exists('agente', $data) && !array_key_exists('agente_id', $data)) {
            $agenteValor = $data['agente'];
            unset($data['agente']);
            if ($agenteValor !== null && $agenteValor !== '') {
                $data['agente_id'] = (int) $agenteValor;
            }
        }
        ...
        return DB::transaction(function () use ($data) {
            ...
            $persona->load(['unidad', 'cliente', 'sucursal', 'dueno', 'transporteTemporal', 'estado', 'agente']);
    ...
    {
        $persona = Persona::with(['unidad', 'cliente', 'sucursal', 'dueno', 'transporteTemporal', 'estado', 'agente'])->findOrFail($id);
    ...
    public function update(PersonaStoreRequest $request, string $id)
    {
        ...
        // Alias 'estado' -> 'estado_id'
        if (array_key_exists('estado', $validated) && !array_key_exists('estado_id', $validated)) {
        ...
        // Alias 'agente' -> 'agente_id'
        if (array_key_exists('agente', $validated) && !array_key_exists('agente_id', $validated)) {
            $agenteValor = $validated['agente'];
            unset($validated['agente']);
            if ($agenteValor !== null && $agenteValor !== '') {
                $validated['agente_id'] = (int) $agenteValor;
            }
        }
